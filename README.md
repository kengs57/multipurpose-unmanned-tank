...
- **수행 역할:** **하드웨어 설계 총괄 & 임베디드 펌웨어 리드 (Hardware & Embedded Firmware Lead)**
  - **기여도:** 웹 GUI 및 3D 모델링을 제외한 **전체 시스템 하드웨어 회로 설계 및 펌웨어 코드 100% 자체 개발**
  - **하드웨어/회로:** 전원 분배망 설계, 대전류 스위칭/구동계 회로 설계 및 제작
  - **임베디드 펌웨어:** 
    - ESP8266(WEMOS D1 R1) 기반 MQTT 통신, 시리얼 패킷 파싱 및 구동 제어 로직 구현
    - Arduino Uno 클러스터 펌웨어 개발 (AVR 레지스터 직접 제어 코일건 점호, LiDAR 계측 파싱, 다축 서보 제어)
  - **협업 구조:** 총 6인 (하드웨어/펌웨어 총괄 1, 기구 조립 보조 3, 웹 프론트엔드 1, 외형 3D 디자인 1)
